export async function callConfiguredLlmJson(
  ...
  try {
    const response = await fetch(url, {
      ...
      body: JSON.stringify({ model, temperature: 0.8, max_tokens: maxTokens, response_format: { type: "json_object" }, messages: [{ role: "system", content: system }, { role: "user", content: user }] }),
    });
